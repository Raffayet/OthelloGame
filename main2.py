...
def initialiseBoard(n):
    ...
    if n < 4:
        print('The size of the board must be at least 4')
        return board
    ...
    if n > 26:
        print('The size of the board must not be greater than 26.')
        return board
...
def saveScore(scores, f, players, size):  # Saves the current set of scores to disk.
    ...
    line = localtime + " " + "Black: " + str(scores[0]) + " White: " + str(scores[1]) + " Players: " + str(
        players) + " Size: " + str(size) + "\n"
    file.write(line)
    file.close()
    return
